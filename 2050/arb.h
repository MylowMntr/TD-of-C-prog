...
typedef struct BinaryTreeElement{  
    int data;
    // le champ donnée d'un noeud, ici une valeur numérique entière
    int nodeID;
    // numéro d'identification d'un noeud. En option, pour des implémentations spécifiques
    struct BinaryTreeElement* leftChild; 
    // pointeur sur le fils gauche
    struct BinaryTreeElement* rightChild; 
    // pointeur sur le fils droit
} BinaryTreeElement;

typedef struct BinaryTree{    
    BinaryTreeElement* anchor; 
    // l'ancrage de l'arbre est réalisé à l'aide d'un noeud fictif dont on n'utilisera arbitrairement le lien vers fils gauche
    int nodeCount;
    // comme pointeur sur la racine de l'arbre. Si celui-ci est NULL alors l'arbre est vide.
} BinaryTree;
...
// utilisé pour implémenter un curseur ou itérateur qui facilite la navigation dans l'arbre.

typedef struct Cursor
{ // un curseur désigne le noeud enfant du côté 'side' du noeudparent référencé par le 'pointeur'
...
// insereune nouvelle valeur dans une nouvelle feuille à la position indiquée par le curseur
// qui doit être en fin (cf estEnFin())// utilisé par la fonction addValueToABR() pour ajouter une nouvelle valeur dans l'arbre
